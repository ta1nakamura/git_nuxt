        ...
        <v-layout row wrap 1>
          <v-flex xs3>
            <v-select v-model = "selected_country" :hint = "`${selected_country.name}, ${selected_country.region}`"
                      ...
                      label = "select country" persistent-hint return-object single-line/>
          </v-flex>
          <v-flex xs4>
            <v-text-field v-model="inputaddress" label="input address"/>
          </v-flex>
        </v-layout>
        <v-btn color="secondary" @click="onGeocode">goMap</v-btn>
        <v-btn color="secondary" @click="onNearSation(1)">最寄りポイント</v-btn>
        <v-btn color="secondary" @click="onNearSation(40)">すべてのポイント</v-btn>
